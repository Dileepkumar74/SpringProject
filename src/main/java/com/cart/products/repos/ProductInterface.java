...
public interface ProductInterface extends JpaRepository<Product, Integer>{
	@Query("SELECT p FROM Product p WHERE p.name LIKE CONCAT('%', :keyword, '%') AND (:filter = 0.0 OR p.price <= :filter)")
	List<Product> searchProducts(@Param("keyword") String keyword, @Param("filter") Double filter, Sort sort);
